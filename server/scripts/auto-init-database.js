...
// Supabase 配置 - 使用 Service Role Key（完全权限）
...
async function initializeDatabase() {
  console.log('=== 开始数据库初始化（使用 Service Role Key）===\n');
  ...
  try {
    ...
    if (existingAdmin) {
      ...
      if (updateError) {
        console.log('  ❌ 更新 admin 用户失败:', updateError.message);
        throw updateError;
      }
    ...
    if (existingPermission) {
      console.log('  ℹ️  admin 用户权限已存在，更新状态...');
      ...
      if (permError) {
        console.log('  ❌ 更新权限失败:', permError.message);
        throw permError;
      }
      console.log('  ✅ admin 用户权限已更新');
    } else {
      ...
      if (permError) {
        console.log('  ❌ 创建权限失败:', permError.message);
        throw permError;
      }
    ...
    if (existingCases && existingCases.length > 0) {
      ...
      const { count } = await supabase.from('medical_cases').select('*', { count: 'exact' });
      console.log(`     当前医案数量: ${count}`);
    } else {
      ...
      for (let i = 0; i < medicalCasesData.length; i++) {
        ...
        successCount++;
        console.log(`  ✅ [${successCount}/${medicalCasesData.length}] 导入成功: ${caseData.prescription_name}`);
      }
    }
    ...
    const { data: cases } = await supabase
      .from('medical_cases')
      .select('prescription_name, doctor_name, diagnosis')
      .order('created_at', { ascending: true });

    if (cases && cases.length > 0) {
      console.log('\n已导入的医案:');
      cases.forEach((c, i) => {
        console.log(`  ${i + 1}. ${c.prescription_name} (${c.doctor_name}) - ${c.diagnosis}`);
      });
    }
    ...
    console.log('  密码: 123456');
    console.log('\n下一步操作：');
    console.log('  1. ✅ 数据库已初始化完成');
    console.log('  2. ⏳ 配置 Render 环境变量');
    console.log('  3. ⏳ 重新部署 Render 服务');
