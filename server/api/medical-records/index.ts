 ...
 */
async function createMedicalRecord(req: NextApiRequest, res: NextApiResponse, user: AuthUser) {
  try {
    const body = req.body;
    const supabase = getSupabaseClient();
    ...
    if (error) {
      console.error('[MedicalRecords] Failed to create record:', error);
      return res.status(500).json({
        code: 500,
        msg: 'error',
        error: '创建失败',
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    return createMedicalRecord(req, res, user!);
